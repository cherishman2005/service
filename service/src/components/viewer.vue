<template>
  <div class="content_wrapper">
    ...
    <div class="sort">
      <ul >
          <li v-for="item in viewer_list" :key="item.id" @click.stop="link(item)">
           <a :href="item.viewer_url">
           <!--<a @click="link(item)">-->
             <div class="sort_main">
    ...
    <div class="footcenter">
      <footer class="mod_footer">
        ...
        <div  class="copyright" ><a href="http://www.yy.cn">广州</a></div>
...
<script>
  ...
  export default {
    ...
    mounted() {
      this.$axios.get(baseURL + '/three_cloud/viewer_list/get').then(res => {
        if (res.status === 200) {
          this.viewer_list = res.data;
    ...
    methods: {
      link(e) {
        let click_params = { uid: this.uid, asset_id: e.viewer_id };
        ...
        this.$axios.post(baseURL + '/three_cloud/click_statistics/inc', JSON.stringify(click_params)).then(res => {
          if (res.status === 200) {
            ...
            window.location.href = e.viewer_url;
          }
        })
        .catch(error => {
        });
